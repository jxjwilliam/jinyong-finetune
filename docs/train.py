...
Bugs fixed vs original:
  1. packing is now read from config and defaults to False — packing=True with
     a chat template corrupts sequence boundaries, teaching the model to ignore
     <|im_start|> tokens.
  2. max_seq_length default bumped to 1024 — 300-char Chinese chunks format to
     ~651 tokens; 512 truncated outputs during training.
  3. build_prompt handles empty user_input cleanly (no trailing \\n).
  4. Evaluation is run every save_steps so loss curves are visible.
  5. gradient_checkpointing enabled to reduce VRAM usage without accuracy loss.
"""
...
def build_prompt(system_prompt: str, instruction: str, user_input: str, output: str) -> str:
    """
    BUG FIX: user_input may be "" for typed pairs.
    Old code: f"...{instruction}\n{user_input}..." → trailing \n when input empty.
    Fixed:    conditionally include input only when non-empty.
    """
...
def main() -> None:
    ...
    # ── Defaults (overridden by anything in YAML) ───────────────────────────
    default_training: dict[str, Any] = {
        ...
        "warmup_ratio": 0.05,
        "num_train_epochs": 2,
        "max_seq_length": 1024,          # BUG FIX: was 512, truncated outputs
        "save_steps": 100,
        "save_total_limit": 3,
        "logging_steps": 10,
        "report_to": "none",
        "fp16": True,
        "bf16": False,
        "packing": False,                # BUG FIX: was True, corrupted chat boundaries
        "eval_split_ratio": 0.05,
        "seed": 42,
        "eval_steps": 100,
        "gradient_checkpointing": True,  # saves ~3 GB VRAM on T4
    }
    ...
    def format_prompt(example: dict[str, str]) -> dict[str, str]:
        return {
            "text": build_prompt(
                system_prompt,
                example["instruction"],
                example.get("input", ""),  # typed pairs have no input
                example["output"],
            )
        }
    ...
    # Sanity check — print token length distribution
    ...
    training_args = SFTConfig(
        ...
        fp16=train_cfg["fp16"],
        bf16=train_cfg["bf16"],
        packing=train_cfg["packing"],            # BUG FIX: False
