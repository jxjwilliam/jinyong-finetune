"""build_instructions.py — fixed version

Bugs fixed vs original:
  1. validate_pairs() dropped ALL typed pairs because input="" failed the
     `if not pair.input.strip()` guard.  Now empty input is valid (typed
     pairs intentionally have no input context).
  2. Typed pairs no longer use a random continuation chunk as their output
     — they are either sourced from a pre-generated Claude/GPT JSONL file
     (--typed-jsonl) or skipped entirely (--skip-typed).
  3. build_prompt in train.py now receives clean input; no trailing \n.
"""
...
try:
    ...
    def clean_novel(text: str) -> str:          # fallback if clean_text absent
        return text
...
class Pair:
    instruction: str
    input: str        # may be "" for typed pairs — that is valid
    output: str
...
# ── Sliding-window continuation pairs ──────────────────────────────────────
...
def load_typed_pairs_from_jsonl(path: Path) -> list[Pair]:
    """
    Load typed pairs that were pre-generated by generate_typed_pairs.py.
...
def validate_pairs(pairs: Iterable[Pair], min_output_chars: int = 50) -> list[Pair]:
    """
    BUG FIX: empty input is now allowed (typed pairs have no input context).
    Only reject if instruction or output is missing/too short.
    """
    validated: list[Pair] = []
    for pair in pairs:
        if not pair.instruction.strip():
            continue
        # input may be "" — that is intentional for typed pairs
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--input-dir", default=None,
                        help="Directory of cleaned .txt novels")
    parser.add_argument("--output", default=None,
                        help="Output JSONL path")
    parser.add_argument("--typed-jsonl", default=None,
                        help="Path to pre-generated typed pairs JSONL "
                             "(from generate_typed_pairs.py). "
                             "If omitted, typed pairs are skipped.")
...
def main() -> None:
    ...
    # ── Continuation pairs (style transfer via sliding window) ──
    ...
    # ── Typed pairs (instruction-following, from pre-generated JSONL) ──
